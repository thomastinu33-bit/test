...
const BRANDS = ["H&M", "Zara", "ASOS", "Uniqlo", "Gap", "Forever 21", "Shein", "TopShop"];
const MODELS = [
...
function generateMockData(): HmHeatmapScoreRow[] {
  ...
  const baseFactor = 0.95; // UK scores ~5% lower
...
export function getHmHeatmapScoresUK(): HmHeatmapScoreRow[] {
...
export function getGaugeDimensionsForBrand(
  ...
  const rows = getHmHeatmapScoresUK();
...
export function getGaugeDimensionsForModelWithChange(
  ...
  if (originalIds.length > 0) {
    ...
    const latestDims = Object.fromEntries(HM_HEATMAP_TOPIC_KEYS.map((k) => [k, 0])) as Record<HmHeatmapTopic, number>;
    ...
    const prevDims = Object.fromEntries(HM_HEATMAP_TOPIC_KEYS.map((k) => [k, 0])) as Record<HmHeatmapTopic, number>;
